...
@section('action-btn')
    <div class="action-btn ms-2">
        <a href="{{route('quote.export')}}" class="btn btn-sm btn-primary btn-icon m-1"data-toggle="tooltip" data-bs-toggle="tooltip" title=" {{__('Export')}}">
    ...
    @can('Create Quote')
        <div class="action-btn ms-2">
            <a href="#" data-url="{{ route('quote.create',['quote',0]) }}" data-size="lg" data-ajax-popup="true" data-bs-toggle="tooltip" data-title="{{__('Create New Quote Item')}}" title="{{__('Create')}}"class="btn btn-sm btn-primary btn-icon m-1">
                    ...
                    <div class="table-responsive overflow_hidden">
                        <table id="datatable" class="table datatable align-items-center">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>                            
                                        <a href="{{ route('quote.show',$quote->id) }}" class="btn btn-outline-primary" data-title="{{__('Quote Details')}}">
                                    ...
                                    <td>
                                        @if($quote->status == 0)
                                            <span class="badge bg-secondary p-2 px-3 rounded" style="width: 79px;">{{ __(\App\Models\Quote::$status[$quote->status]) }}</span>
                                        ...
                                        @elseif($quote->status == 1)
                                            <span class="badge bg-info p-2 px-3 rounded" style="width: 79px;">{{ __(\App\Models\Quote::$status[$quote->status]) }}</span>
                                        @endif
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <span class="budget">{{\Auth::user()->priceFormat($quote->getTotal())}}</span>
                                    </td>
                                    <td>
                                        ...
                                        <td class="text-end">
                                            @can('Create Quote')
                                                <div class="action-btn bg-secondary ms-2">
                                                    {!! Form::open(['method' => 'get', 'route' => ['quote.duplicate', $quote->id],'id'=>'duplicate-form-'.$quote->id]) !!}

                                                    <a href="#" class="mx-3 btn btn-sm align-items-center text-white duplicate_confirm" data-bs-toggle="tooltip" data-title="{{__('Duplicate')}}"  title="{{__('Duplicate')}}" data-confirm="You want to confirm this action. Press Yes to continue or Cancel to go back" data-confirm-yes="document.getElementById('duplicate-form-{{$quote->id}}').submit();">
                                            ...
                                            @if($quote->converted_salesorder_id == 0)
                                                <div class="action-btn bg-success ms-2">
                                                    ...
                                                    <a href="#" class="mx-3 btn btn-sm align-items-center text-white convert_confirm" data-bs-toggle="tooltip" data-title="{{__('Convert to Sales Order')}}"  title="{{__('Conver to Sale Order')}}" data-confirm="{{__('You want to confirm convert to sales order. Press Yes to continue or Cancel to go back')}}" data-confirm-yes="document.getElementById('quotes-form-{{$quote->id}}').submit();">
                                            ...
                                            @else
                                                <div class="action-btn bg-success ms-2">
                                                    <a href="{{ route('salesorder.show',$quote->converted_salesorder_id) }}" class="mx-3 btn btn-sm align-items-center text-white" data-bs-toggle="tooltip" data-original-title="{{__('Sales Order Details')}}" title="{{__('SalesOrders Details')}}">
                                            ...
                                            @can('Show Quote')
                                                <div class="action-btn bg-warning ms-2">
                                                    <a href="{{ route('quote.show',$quote->id) }}" data-size="md"class="mx-3 btn btn-sm align-items-center text-white " data-bs-toggle="tooltip" title="{{__('Details')}}" data-title="{{__('Quote Details')}}">
                                                        <i class="ti ti-eye"></i>
                                                    </a>    
                                                </div>
                                            @endcan
                                            @can('Edit Quote')
                                                <div class="action-btn bg-info ms-2">
                                                    <a href="{{ route('quote.edit',$quote->id) }}" class="mx-3 btn btn-sm align-items-center text-white" data-bs-toggle="tooltip" title="{{__('Edit')}}" data-title="{{__('Edit Quote')}}"><i class="ti ti-edit"></i></a>
                                                </div>
                                            @endcan
                                            @can('Delete Quote')
