...
<div class="action-btn bg-warning ms-2">
    ...
    @if(Auth::user()->type == 'owner')
        <div class="action-btn ms-2">
            <a href="#" class="btn btn-sm btn-warning btn-icon m-1 cp_link" data-link="{{route('pay.quote',\Illuminate\Support\Facades\Crypt::encrypt($quote->id))}}" data-bs-toggle="tooltip" data-title="{{__('Click to copy Quote link')}}"title="{{__('copy quote')}}"><span class="btn-inner--icon text-white"><i class="ti ti-file"></i></span></a>
        </div>
    @endif
    @can('Edit Quote')
        <div class="action-btn ms-2">
            <a href="{{ route('quote.edit',$quote->id) }}" class="btn btn-sm btn-info btn-icon m-1" data-bs-toggle="tooltip" title="{{__('Edit')}}" data-title="{{__('Edit Quote')}}"><i class="ti ti-edit"></i></a>
        </div>
    @endcan
    <div class="action-btn ms-2">
        <a href="#" data-url="{{ route('quote.quoteitem',$quote->id) }}" data-size="md" data-ajax-popup="true" data-bs-toggle="tooltip" data-title="{{__('Create New Quote Item')}}"title="{{__('Create')}}" class="btn btn-sm btn-primary btn-icon m-1">
                        ...
                        <div class="row align-items-center mb-4">
                            <div class="col-sm-6 mb-3 mb-sm-0">
                                ...
                                <div class="col-lg-6 col-md-8 pl-0 mt-3">
                                    ...
                                    <span class="col-sm-8"><span class="text-sm">{{\Auth::user()->dateFormat($quote->created_at)}}</span></span>
                                </div>
                                    ...
                                    <table class="table invoice-detail-table">
                                        ...
                                        <tbody>
                                            ...
                                            @foreach($quote->items as $quoteitem)
                                                ...
                                                @endphp
                                                <tr>
                                                    <td>{{(!empty($quoteitem->items->name)?$quoteitem->items->name:'')}} </td>
                                                        ...
                                                        <div class="col">
                                                            @if(!empty($quoteitem->tax))
                                                                @foreach($quoteitem->tax($quoteitem->tax) as $tax)
                                                                    @php
                                                                        $taxPrice=\Utility::taxRate(!empty($tax->rate),$quoteitem->price,$quoteitem->quantity);
                                                                        $totalTaxPrice+=$taxPrice;
                                                                    @endphp
                                                                    <a href="#!" class="d-block text-sm text-muted">{{$tax->tax_name??'No Tax' .' ('.!empty($tax->rate) .'%)'}} &nbsp;&nbsp;{{\Auth::user()->priceFormat($taxPrice)}}</a>
                                                                @endforeach
                                                            @else
                                                    ...
                                                    <td>
                                                        ...
                                                        <div class="action-btn bg-info ms-2">
                                                            <a href="#" data-url="{{ route('quote.quoteitem.edit',$quoteitem->id) }}" data-ajax-popup="true" class="mx-3 btn btn-sm d-inline-flex align-items-center text-white" data-bs-toggle="tooltip" title="{{__('Edit')}}" data-title="{{__('Edit Quote')}}"><i class="ti ti-edit"></i></a>
                                                        </div>
                                                            {{-- <a href="#" data-url="{{ route('quote.quoteitem.edit',$quoteitem->id) }}" data-ajax-popup="true" class="mx-3 btn btn-sm d-inline-flex align-items-center text-white" data-toggle="tooltip" data-original-title="{{__('Edit')}}" data-title="{{__('Edit Quote')}}"><i class="ti ti-edit"></i></a> --}}
                                                        @endcan
                                                        @can('Delete Quote')
                                    ...
                                    <table class="table invoice-table ">
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <hr />
                                                    <h5 class="text-primary subTotal">  {{\Auth::user()->priceFormat($quote->getTotal())}}</h5>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
