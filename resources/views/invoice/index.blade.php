    ...
    @can('Create Invoice')
        <div class="action-btn ms-2">
            <a href="#" data-size="lg" data-url="{{ route('invoice.create', ['invoice', 0]) }}" data-ajax-popup="true"
                data-bs-toggle="tooltip" data-title="{{ __('Create New Invoice Item') }}" title=" {{ __('Create') }}"
                                ...
                                @foreach ($invoices as $invoice)
                                    <tr>
                                        ...
                                        <td>
                                            @if ($invoice->status == 0)
                                                <span
                                                    class="badge bg-secondary p-2 px-3 rounded" style="width: 91px;" >{{ __(\App\Models\Invoice::$status[$invoice->status]) }}</span>
                                            @elseif($invoice->status == 1)
                                                <span
                                                    class="badge bg-danger p-2 px-3 rounded" style="width: 91px;">{{ __(\App\Models\Invoice::$status[$invoice->status]) }}</span>
                                            @elseif($invoice->status == 2)
                                                <span
                                                    class="badge bg-warning p-2 px-3 rounded" style="width: 91px;">{{ __(\App\Models\Invoice::$status[$invoice->status]) }}</span>
                                            @elseif($invoice->status == 3)
                                                <span
                                                    class="badge bg-success p-2 px-3 rounded" style="width: 91px;">{{ __(\App\Models\Invoice::$status[$invoice->status]) }}</span>
                                            @elseif($invoice->status == 4)
                                                <span
                                                    class="badge bg-info p-2 px-3 rounded" style="width: 91px;">{{ __(\App\Models\Invoice::$status[$invoice->status]) }}</span>
                                            @endif
                                        </td>
                                        <td>
                                            <span
                                        ...
                                        @if (Gate::check('Show Invoice') || Gate::check('Edit Invoice') || Gate::check('Delete Invoice'))
                                            <td class="text-end">
                                                ...
                                                @can('Delete Invoice')
                                                    ...
                                                    {{-- <a href="#" class="btn  btn-icon btn-danger px-1  " data-toggle="tooltip" data-original-title="{{__('Delete')}}" data-confirm="{{__('Are You Sure?').' | '.__('This action can not be undone. Do you want to continue?')}}" data-confirm-yes="document.getElementById('delete-form-{{$invoice->id}}').submit();">
                                                <i class="ti ti-trash"></i>
                                            </a>
