    ...
    @if (Auth::user()->type == 'owner')
        <div class="action-btn bg-warning ms-2">
            <a href="#" class="btn btn-sm btn-warning btn-icon m-1 cp_link"
                ...
                title="{{ __('Copy invoice') }}"><span class="btn-inner--icon text-white"><i class="ti ti-file"></i></span></a>
        </div>
    @endif
    ...
    @can('Edit Invoice')
        <div class="action-btn ms-2">
            <a href="{{ route('invoice.edit', $invoice->id) }}"
                class="btn btn-sm btn-info btn-icon m-1" data-bs-toggle="tooltip"
                data-title="{{ __('invoice Edit') }}" title="{{ __('Edit') }}"><i class="ti ti-edit"></i>
            </a>
        </div>
    @endcan
    <div class="action-btn ms-2">
        <a href="#" data-size="md" data-url="{{ route('invoice.invoiceitem', $invoice->id) }}" data-ajax-popup="true"
                    ...
                    <div class="card-body">
                        <div class="row align-items-center mb-4">
                            ...
                            <div class="col-lg-6 col-md-8 mt-3">
                                ...
                                <span class="col-sm-8"><span class="text-sm">{{\Auth::user()->dateFormat($invoice->created_at)}}</span></span>
                            </div>
                                <h6 class="d-inline-block m-0 d-print-none mt-3">{{__('Invoice')}}</h6>
                                @if($invoice->status == 0)
                                    <span class="badge bg-primary p-2 px-3 rounded">{{ __(\App\Models\Invoice::$status[$invoice->status]) }}</span>
                                @elseif($invoice->status == 1)
                                    <span class="badge bg-danger p-2 px-3 rounded">{{ __(\App\Models\Invoice::$status[$invoice->status]) }}</span>
                                @elseif($invoice->status == 2)
                                    <span class="badge bg-warning p-2 px-3 rounded">{{ __(\App\Models\Invoice::$status[$invoice->status]) }}</span>
                                @elseif($invoice->status == 3)
                                    <span class="badge bg-success p-2 px-3 rounded">{{ __(\App\Models\Invoice::$status[$invoice->status]) }}</span>
                                @elseif($invoice->status == 4)
                                    <span class="badge bg-info p-2 px-3 rounded">{{ __(\App\Models\Invoice::$status[$invoice->status]) }}</span>
                                @endif
                            </div>
                                    ...
                                    <table class="table invoice-detail-table">
                                        ...
                                        @foreach ($invoice->items as $invoiceitem)
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-right">
                                                    {{ \Auth::user()->priceFormat($invoiceitem->price * $invoiceitem->quantity) }}
                                                </td>
                                                <td class="text-right">
                                                    @can('Edit Invoice')
                                                        ...
                                                        {{-- <a href="#" data-url="{{ route('invoice.item.edit',$invoiceitem->id) }}" data-ajax-popup="true" class="mx-3 btn btn-sm d-inline-flex align-items-center text-white " data-bs-toggle="tooltip" data-original-title="{{__('Edit')}}" title="{{__('Edit Item')}}"><i class="far fa-edit"></i></a> --}}
                                                    @endcan
                                                    @can('Delete Invoice')
                                    ...
                                    <table class="table invoice-table ">
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <hr />
                                                    <h5 class="text-primary subTotal">  {{ \Auth::user()->priceFormat($invoice->getTotal()) }}</h5>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 col-md-4">
                                ...
                                <dl class="row mt-4 align-items-center">
                                    ...
                                    <dd class="col-sm-6"><span class="text-sm">{{ $invoice->billing_address }}</span></dd>
                                    ...
                                    <dt class="col-sm-6"><span class="h6 text-sm mb-0">{{__('Zip Code') }}</span></dt>
                                    <dd class="col-sm-6"><span class="text-sm">{{ $invoice->billing_postalcode }}</span></dd>

                                    <dt class="col-sm-6"><span class="h6 text-sm mb-0">{{__('Billing Country')}}</span></dt>
                                    <dd class="col-sm-6"><span class="text-sm">{{ $invoice->billing_country }}</span></dd>


                                    <dt class="col-sm-6"><span class="h6 text-sm mb-0">{{__('Billing Contact')}}</span></dt>
                                    <dd class="col-sm-6"><span class="text-sm">{{ !empty($invoice->contacts->name)?$invoice->contacts->name:'--'}}</span></dd>
                                </dl>
                            </div>
                            <div class="col-12 col-md-4">
                                ...
                                <dl class="row mt-4 align-items-center">
                                    ...
                                    <dd class="col-sm-6"><span class="text-sm">{{ $invoice->shipping_address }}</span></dd>

                                    <dt class="col-sm-6"><span class="h6 text-sm mb-0">{{__('Shipping City')}}</span></dt>
                                    <dd class="col-sm-6"><span class="text-sm">{{ $invoice->shipping_city }}</span></dd>

                                    <dt class="col-sm-6"><span class="h6 text-sm mb-0">{{__('Zip Code')}}</span></dt>
                                    <dd class="col-sm-6"><span class="text-sm">{{ $invoice->shipping_postalcode }}</span></dd>

                                    <dt class="col-sm-6"><span class="h6 text-sm mb-0">{{__('Shipping Country')}}</span></dt>
                                    <dd class="col-sm-6"><span class="text-sm">{{ $invoice->shipping_country }}</span></dd>
