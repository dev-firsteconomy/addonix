...
<div class="action-btn bg-warning ms-2">
    ...
    @if(Auth::user()->type == 'owner')
        <div class="action-btn ms-2">
            <a href="#" class="btn btn-sm btn-warning btn-icon m-1 cp_link" data-link="{{route('pay.salesorder',\Illuminate\Support\Facades\Crypt::encrypt($salesOrder->id))}}" data-bs-toggle="tooltip" data-title="{{__('Click to copy SalesOrder link')}}"title="{{__('copy salesorder')}}"><span class="btn-inner--icon text-white"><i class="ti ti-file"></i></span></a>
        </div>
    @endif
    @can('Edit SalesOrder')
        <div class="action-btn ms-2">
            <a href="{{ route('salesorder.edit',$salesOrder->id) }}" class="btn btn-sm btn-info btn-icon m-1" data-bs-toggle="tooltip" data-title="{{__('Sales order Edit')}}" title="{{__('Edit')}}"><i class="ti ti-edit"></i>
            </a>
        </div>
    @endcan
    <div class="action-btn ms-2">
        <a href="#" data-size="md" data-url="{{ route('salesorder.salesorderitem',$salesOrder->id) }}" data-ajax-popup="true" data-bs-toggle="tooltip" data-title="{{__('Create New SalesOrder')}}" title="{{__('Create')}}" class="btn btn-sm btn-primary btn-icon m-1">
            ...
            <div>
                    ...
                    <div class="card-body">
                        <div class="row align-items-center mb-4">
                            <div class="col-sm-6 mb-3 mb-sm-0">
                                ...
                                <span class="col-sm-8"><span class="text-sm">{{ \Auth::user()->salesorderNumberFormat($salesOrder->id) }}</span></span>
                            </div>
                            <div class="col-lg-6 col-md-8 mt-3">
                                <h6 class="d-inline-block m-0 d-print-none">{{__('Sales Order Date')}}</h6>
                                <span class="col-sm-8"><span class="text-sm">{{\Auth::user()->dateFormat($salesOrder->created_at)}}</span></span>
                            </div>
                            ...
                            <div class="col-sm-6 text-sm-end">
                                ...
                                <div>
                                    <div class="float-end mt-3">
                                        {!! DNS2D::getBarcodeHTML(route('pay.salesorder',\Illuminate\Support\Facades\Crypt::encrypt($salesOrder->id)), "QRCODE",2,2) !!}
                                    </div>
                                ...
                                <div class="table-responsive mt-4">
                                    <table class="table invoice-detail-table">
                                        ...
                                        @foreach($salesOrder->items as $salesOrderitem)
                                            ...
                                            @endphp
                                            <tr>
                                                <td>{{!empty($salesOrderitem->items->name)?$salesOrderitem->items->name:'--'}} </td>
                                                ...
                                                <td>{{\Auth::user()->priceFormat($salesOrderitem->discount)}} </td>
                                                <td >{{!empty($salesOrderitem->description)?$salesOrderitem->description:'--'}} </td>
                                                <td class="text-right"> {{\Auth::user()->priceFormat($salesOrderitem->price*$salesOrderitem->quantity)}}</td>
                                                <td class="text-right">
                                                    @can('Edit SalesOrder')
                                                    <div class="action-btn bg-info ms-2">
                                                        <a href="#" data-url="{{ route('salesorder.item.edit',$salesOrderitem->id) }}" data-ajax-popup="true" class="mx-3 btn btn-sm d-inline-flex align-items-center text-white" data-bs-toggle="tooltip" data-original-title="{{__('Edit')}}" title="{{__('Edit Item')}}"><i class="ti ti-edit"></i></a>
                                                    </div>
                                                        {{-- <a href="#" data-url="{{ route('salesorder.item.edit',$salesOrderitem->id) }}" data-ajax-popup="true" class="mx-3 btn btn-sm d-inline-flex align-items-center text-white" data-bs-toggle="tooltip" data-original-title="{{__('Edit')}}" title="{{__('Edit Item')}}"><i class="far fa-edit"></i></a> --}}
                                                    @endcan
                                                    ...
                                                    {!! Form::close() !!}
                                                        ...
                                                        {{-- <a href="#" class="action-item " data-toggle="tooltip" data-original-title="{{__('Delete')}}" data-confirm="{{__('Are You Sure?').' | '.__('This action can not be undone. Do you want to continue?')}}" data-confirm-yes="document.getElementById('delete-form-{{$salesOrderitem->id}}').submit();">
                                                            <i class="fas fa-trash"></i>
                                                        </a>
                                                        {!! Form::open(['method' => 'DELETE', 'route' => ['salesorder.items.delete', $salesOrderitem->id],'id'=>'delete-form-'.$salesOrderitem->id]) !!}
                                                        {!! Form::close() !!} --}}
                                                    @endcan
                                                </td>
                                                @php
                                ...
                                <div class="invoice-total">
                                    <table class="table invoice-table ">
                                        <tbody>
                                            ...
                                            <tr>
                                                <th>Discount :</th>
                                                <td>{{\Auth::user()->priceFormat($salesOrder->getTotalDiscount())}}</td>
                                            </tr>
                                            <tr>
                                                <td>
                                                    <hr />
                                                ...
                                                <td>
                                                    <hr />
                                                    <h5 class="text-primary subTotal"> {{\Auth::user()->priceFormat($salesOrder->getTotal())}}</h5>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
                ...
                {{-- <div class="row text-center d-print-none">
                    <div class="col-sm-12 invoice-btn-group text-center mb-3">
                        <button type="button"
                            class="btn btn-print-invoice  btn-primary m-b-10 m-r-10">Print</button>
                        <button type="button" class="btn  btn-secondary m-b-10 ">Cancel</button>
                        <h3 class="mt-4">Thank you!</h3>
                        <p class="mb-0">If you encounter any issues related to the invoice you can contact us at:</p>
                        <p>email: <a href="mailto:abc@mail.com"></a> support@mail.com</p>
                    </div>
                </div> --}}
            </div>
        </div>
                ...
                <ul class="list-group list-group-flush">
                    <li class="list-group-item px-0">
                        <div class="row align-items-center">
                            ...
                            <dd class="col-sm-12"><span class="text-sm">{{\Auth::user()->dateFormat($salesOrder->created_at)}}</span></dd>
                        </div>
                    </li>
                </ul>
            </div>
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    for (var i = 0; i < invoiceItems.taxes.length; i++) {
                        taxes += '<span class="badge bg-primary ms-1 mt-1">' + invoiceItems.taxes[i].tax_name + ' ' + '(' + invoiceItems.taxes[i].rate + '%)' + '</span>';
                    }
