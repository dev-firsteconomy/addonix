...
<div class="col-md-6 float-end d-flex align-items-center justify-content-end">
    ...
    @if (\Auth::user()->type == 'owner' && \Auth::user()->type != 'Manager')
        @if( $contract->status == 'accept')
            <div class="action-btn ms-2">
                <a href="#" data-size="lg" data-url="{{route('contracts.copy',$contract->id)}}" data-ajax-popup="true" data-title="{{__('Duplicate Contract')}}" class="btn btn-sm btn-primary btn-icon" data-bs-toggle="tooltip" data-bs-placement="top" title="{{__('Duplicate')}}" ><i class="ti ti-copy text-white"></i></a>
            </div>
        @endif
    @endif
    ...
    @if(((\Auth::user()->type =='owner') && ($contract->owner_signature == '')||(\Auth::user()->type =='Manager') && ($contract->client_signature == '') )&& ($contract->status == 'accept'))
        <div class="action-btn ms-2">
            <a href="#" data-size="md"class="btn btn-sm btn-primary btn-icon" data-url="{{ route('signature',$contract->id) }}" data-ajax-popup="true" data-title="{{__('Signature')}}" data-size="lg" title="{{__('Signature')}}" data-bs-toggle="tooltip" data-bs-placement="top">
                                    ...
                                    <div class="card-body pt-0 pb-0" >
                                        ...
                                        {{-- <address class="mb-0 text-sm"> --}}
                                            <dl class="row mt-2  align-items-center">
                                                ...
                                                <dt class="col-sm-5 h6 text-sm">{{ __('Client Name') }}</dt>
                                                <dd class="col-sm-5 text-sm"> {{ $contract->client->name }}</dd>
                                                ...
                                                <dd class="col-sm-5 text-sm"> {{Auth::user()->priceFormat($contract->value) }}</dd>
                                                <dt class="col-sm-5 h6 text-sm">{{__('Start Date')}}</dt>
                                                <dd class="col-sm-5 text-sm">{{ Auth::user()->dateFormat($contract->start_date) }}</dd>
                                                <dt class="col-sm-5 h6 text-sm">{{__('End Date')}}</dt>
                                                <dd class="col-sm-5 text-sm">{{ Auth::user()->dateFormat($contract->end_date) }}</dd>
                                                {{-- <dt class="col-sm-5 h6 text-sm">{{__('Status')}}</dt>
                                                <dd class="col-sm-5 text-sm">{{ $contract->status}}</dd> --}}
                                            </dl>
                                        {{-- </address> --}}
                                    </div>
                                </div>
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        {{ Form::open(['route' => ['contracts.description.store', $contract->id]]) }}
                                            ...
                                            @if( $contract->status == 'accept' && \Auth::user()->type == 'owner')
                                    ...
                                    <div class="action-btn bg-warning p-0 w-auto    ">
                                        ...
                                        @if( $contract->status == 'accept')
                                            <div class="col-auto actions">
                                                @if( \Auth::user()->type == 'Manager' && \Auth::user()->id == $file->user_id)
                                                    ...
                                                    <div class="list-group-item px-0">
                                                        <div class="row align-items-center">
                                                            ...
                                                            {{-- @dd($comment) --}}
                                                                ...
                                                                <a href="{{ !empty($user->avatar) ? $profile . '/' . $user->avatar : $profile . '/avatar.png' }}" target="_blank" class="avatar avatar-sm rounded-circle">
                                                                    <img  class="rounded-circle"  width="50" height="50" src="{{ !empty($user->avatar) ? $profile . '/' . $user->avatar : $profile . '/avatar.png' }}">
                                                                </a>
                                                            </div>
                                                            <div class="col ml-n2">
                                                                ...
                                                                <small class="d-block">{{$comment->created_at->diffForHumans()}}</small>
                                                                 ...
                                                                 @if( $contract->status == 'accept')
                                                                    <div class="col-auto">
                                                                        @if( \Auth::user()->type == 'Manager' && \Auth::user()->id == $comment->user_id)
                                                                        <div class="col-auto p-0 mx-3 ms-2 action-btn bg-danger">
                                                ...
                                                <div class="list-group-item ">
                                                    <div class="row align-items-center">
                                                        <div class="col-auto">
                                                            ...
                                                            <a href="{{ !empty($user->avatar) ? $profiles . '/' . $user->avatar : $profiles . '/avatar.png' }}" target="_blank" class="avatar avatar-sm rounded-circle">
                                                                <img class="rounded-circle" width="50" height="50" src="{{ !empty($user->avatar) ? $profiles . '/' . $user->avatar : $profiles . '/avatar.png' }}" title="{{ $contract->client->name }}">
                                                            </a>
                                                        </div>
                                                        <div class="col ml-n2">
                                                            ...
                                                            @if( $contract->status == 'accept')
                                                                <div class="col-auto">
                                                                    @if( \Auth::user()->type == 'Manager' && \Auth::user()->id == $note->user_id)
