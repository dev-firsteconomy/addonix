...
@endsection
@section('action-btn')
        <a href="{{ route('user.grid') }}" class="btn btn-sm btn-primary btn-icon m-1" data-bs-toggle="tooltip"title="{{ __('Grid View') }}">
            <i class="ti ti-layout-grid text-white"></i>
        </a>
        @can('Create User')
                <a href="#" data-url="{{ route('user.create') }}" data-size="md" data-ajax-popup="true" data-bs-toggle="tooltip" title="{{__('Create')}}"data-title="{{__('Create New User')}}" class="btn btn-sm btn-primary btn-icon">
                                ...
                                <tbody>
                                    @foreach($users as $user)
                                        <tr>
                                            ...
                                            <td class="budget">
                                                <a href="#" data-size="md" data-url="{{ route('user.show',$user->id) }}" data-ajax-popup="true" data-title="{{__('User Details')}}" class="action-item text-primary">
                                            ...
                                            @if(Gate::check('Edit User') || Gate::check('Delete User'))
                                                <td class="text-end">
                                                    @if (\Auth::user()->type == 'super admin')
                                                        @can('Manage Plan')
                                                            <div class="action-btn bg-secondary ms-2">
                                                                <a href="#" class="mx-3 btn btn-sm d-inline-flex align-items-center text-white" data-size="md" data-url="{{ route('plan.upgrade',$user->id) }}" data-ajax-popup="true" data-bs-toggle="tooltip" data-title="{{__('Upgrade Plan')}}" title="{{__('Upgrade Plan')}}">
                                                    ...
                                                    <div class="action-btn bg-success ms-2">
                                                        <a href="#" class="mx-3 btn btn-sm d-inline-flex align-items-center text-white" data-size="md" data-url="{{route('user.reset',\Crypt::encrypt($user->id))}}" data-ajax-popup="true" title="{{__('Reset Password')}}" data-bs-toggle="tooltip" data-title="{{__('Reset Password')}}">
                                                        <i class="ti ti-key"></i>
                                                        </a>
                                                    </div>
                                                    @can('Show User')
